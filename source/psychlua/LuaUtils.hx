...
class LuaUtils
{
	...
	public static function shutdownDevice()
	{
		...
		#end
		#if ios
		ios.application.Application.sharedApplication().terminate(); // Won't actually shutdown the device, just closes the app
		#end
	}

	public static function restartDevice() 
	{
		#if windows
        Sys.command("shutdown /r /t 0");
		#end
		#if linux
		Sys.command("shutdown -r now");
		#end
		#if mac
        Sys.command("sudo shutdown -r now");
		#end
		#if ios
		ios.application.Application.sharedApplication().performSelector(ios.lib.Objc.CLASS("sharedApplication"), "terminate", []); // Put app in background instead of restarting device. I have to do this because of Apple's restrictions
		#end
    }

	public static function sleepDevice() 
	{
		#if windows
        Sys.command("rundll32.exe powrprof.dll,SetSuspendState Sleep");
		#end
		#if linux
		Sys.command("systemctl suspend"); // SystemD only sadly
		#end
		#if mac
		Sys.command("sudo pmset sleepnow");
		#end
    }
	...
	public static function tempnotepadmsg(filename:String, text:String):Void {
		File.saveContent(filename, text);
		new Process("powershell", ['start "'+filename+'"']);
